...
import { PenTool, Calendar, Settings, Sun, Moon, X, Lock, ChevronLeft, ChevronRight, Trash2, Info, LogIn, LogOut, UserPlus, Archive, Mail, Mic, StopCircle, Play, Pause } from 'lucide-react'
...
export default function EllaApp() {
  ...
  const viewSelectedEntry = (entryId: string) => {
    ...
    if (entry) {
      ...
      setShowNewEntry(true)
      if (entry.audio) {
        setAudioBlob(new Blob([entry.audio], { type: 'audio/webm' }))
      }
    }
  }
  ...
  const getSuggestedPrompt = () => {
    ...
    const recentMoods = entries.slice(0, 10).map(entry => entry.mood)
    ...
    const recentPrompts = entries.slice(0, 5).flatMap(entry => entry.chat.filter(msg => msg.type === 'prompt').map(msg => msg.content))
    const unusedPrompts = promptsForMood.filter(prompt => !recentPrompts.includes(prompt))

    let suggestedPrompt = ''

    if (Math.random() < 0.3 && lastWeekEntry) {
      if (lastWeekEntry.mood === currentMood) {
        suggestedPrompt = `Last ${getDayName(dayOfWeek)}, you were feeling ${currentMood.toLowerCase()} as well. How does this ${timeOfDay} compare?`
      } else {
        suggestedPrompt = `Last ${getDayName(dayOfWeek)}, you were feeling ${lastWeekEntry.mood.toLowerCase()}. What's different about this ${timeOfDay}?`
      }
    } else if (Math.random() < 0.5 && mostFrequentMood !== currentMood) {
      suggestedPrompt = `You've been feeling ${mostFrequentMood.toLowerCase()} lately, but today you're ${currentMood.toLowerCase()}. What changed?`
    } else {
      const moodBasedPrompts = {
        'Great': [`What's making this ${timeOfDay} particularly great?`, `How can you spread your positive energy today?`],
        'Good': [`What's one thing you're looking forward to this ${timeOfDay}?`, `How can you make this good day even better?`],
        'OK/Meh': [`What could turn this ${timeOfDay} from OK to good?`, `What's one small positive thing you've noticed today?`],
        'Low': [`What's one small thing you could do to lift your spirits this ${timeOfDay}?`, `Who could you reach out to for support right now?`],
        'Doomed': [`What's one thing you can control in this moment?`, `How can you show yourself some compassion this ${timeOfDay}?`]
      }

      const contextualPrompts = [...(moodBasedPrompts[currentMood] || []), ...(unusedPrompts.length > 0 ? [unusedPrompts[Math.floor(Math.random() * unusedPrompts.length)]] : [])]
      suggestedPrompt = contextualPrompts[Math.floor(Math.random() * contextualPrompts.length)]
    }

    return suggestedPrompt || "How are you feeling right now? Let's explore that."
  }
    ...
    <div className={`min-h-screen flex transition-all duration-500 ${
      ...
      <div className="flex-1 flex flex-col p-4 md:p-8">
        <header className="flex justify-between items-center mb-8">
          ...
          <h1 className={`text-5xl font-bold ${theme === 'dark' ? 'text-white' : 'text-white'} ${abrilFatface.className} text-center flex-grow`}>ella</h1>
          <div className="w-1/3" /> {/* Placeholder for symmetry */}
        </header>
        <main className="flex-1 flex flex-col items-center max-w-2xl mx-auto w-full">
          <h2 className={`text-3xl font-bold mb-6 ${theme === 'dark' ? 'text-white' : 'text-white'} font-['Montserrat', 'Outfit', sans-serif] text-center`}>
    ...
    <div className={`rounded-lg p-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white bg-opacity-20'}`}>
      ...
      <div className="flex justify-between mt-4">
        <motion.button
          ...
          }I'll update the Ella Journal app to include audio recording functionality for rants, with a 2-minute limit per recording, and store them locally. We'll use the Web Audio API for recording and IndexedDB for local storage. I'll modify the `EllaApp` component to include this new feature.



```tsx project="Ella Journal" file="app.tsx" type="react"
'use client'
        ...
        {!isViewingEntry && (
          <>
            <motion.button
              ...
              } font-['Open Sans', sans-serif] flex items-center`}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
