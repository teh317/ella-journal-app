...
export default function EllaApp() {
  ...
  const getSuggestedPrompt = () => {
    ...
    if (lastWeekEntry) {
      return `Last ${getDayName(dayOfWeek)}, you mentioned feeling ${lastWeekEntry.mood.toLowerCase()}. How are you feeling today?`;
    }
    ...
    if (mostFrequentMood) {
      return `I've noticed you've been feeling ${mostFrequentMood.toLowerCase()} lately. ${getRandomPrompt(currentMood)}`;
    }

    return getRandomPrompt(currentMood);
  };

  const getRandomPrompt = (mood: Mood) => {
  ...
  const getMostFrequentMood = (moods: Mood[]) => {
    const moodCounts = moods.reduce((acc, mood) => {
      acc[mood] = (acc[mood] || 0) + 1;
      return acc;
    }, {} as Record<Mood, number>);

    return Object.entries(moodCounts).reduce((a, b) => a[1] > b[1] ? a : b)[0] as Mood;
  };
        ...
        <div className="flex-1 flex flex-col p-4">
          ...
          <main className="flex-1 flex flex-col max-w-2xl mx-auto w-full">
            ...
            <div className="flex items-center justify-center mb-2">
              ...
              <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-white'} opacity-50 font-['Montserrat', 'Outfit', sans-serif]`}>
  ...
  return (
    <div className={`h-full p-4 overflow-y-auto ${theme === 'dark' ? 'bg-gray-800' : 'bg-white bg-opacity-10'} ${
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <h2 className="text-white font-bold font-['Montserrat', 'Outfit', sans-serif]">
      ...
      <div className="grid grid-cols-7 gap-1 mb-4">
        {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(day => (
          <div key={day} className="text-center text-white text-opacity-60 font-['Montserrat', 'Outfit', sans-serif]">{day}</div>
        ))}
        ...
        {Array.from({ length: daysInMonth }).map((_, index) => {
          ...
          return (
            <button
              ...
              } font-['Montserrat', 'Outfit', sans-serif]`}
      ...
      <div className="mt-4 space-y-4">
        <h3 className="text-lg font-semibold text-white font-['Montserrat', 'Outfit', sans-serif]">Recent Entries</h3>
        ...
        {entries.slice(0, 5).map((entry) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-white text-opacity-60 font-['Montserrat', 'Outfit', sans-serif]">
                {new Date(entry.date).toLocaleDateString()}
              </span>
            </div>
            <p className="text-white text-sm mt-1 truncate font-['Montserrat', 'Outfit', sans-serif]">
              {entry.chat[entry.chat.length - 1].content}
            </p>
          </div>
        ))}
      </div>
      <div className="mt-4 text-white text-sm font-['Montserrat', 'Outfit', sans-serif]">
...
function AdSpace({ theme }: { theme: string }) {
  return (
    <div className={`my-4 p-4 rounded-lg relative ${
      ...
      <span className="absolute top-1 left-1 text-xs opacity-50 font-['Montserrat', 'Outfit', sans-serif]">Ad</span>
      <div className="h-20 flex items-center justify-center text-white text-opacity-50 font-['Montserrat', 'Outfit', sans-serif]">
        Advertisement Space
      </div>
    </div>
  );
...
function SettingsModal({ userName, userEmail, saveUserProfile, closeModal, theme, setTheme, deleteEntries }: {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className={`p-6 rounded-lg ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} w-full max-w-md max-h-[90vh] overflow-y-auto`}>
        <h2 className={`text-2xl font-bold mb-4 ${theme === 'dark' ? 'text-white' :  'text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}>Settings</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className={`block mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}>Name</label>
            <input
              type="text"
              id="name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              className={`w-full p-2 rounded ${theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}
            />
          </div>
          <div>
            <label htmlFor="email" className={`block mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}>Email</label>
            <input
              type="email"
              id="email"
              value={email}
          ...
          <div>
            <label className={`block mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}>Theme</label>
            <select
              value={theme}
          ...
          <div className="mt-8 mb-8">
            <h3 className={`text-xl font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit_Thin', sans-serif]`}>About Ella</h3>
            <p className={`${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit_Thin', sans-serif]`}>
          ...
          <div className="mt-8">
            <h3 className={`text-xl font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}>Danger Zone</h3>
            {showDeleteConfirmation ? (
              <div className="space-y-2">
                <p className={`${theme === 'dark' ? 'text-white' : 'text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}>Are you sure you want to delete all entries? This action cannot be undone.</p>
                <div className="flex space-x-2">
                  <button
                    onClick={handleDeleteEntries}
                    className="px-4 py-2 rounded bg-red-600 text-white font-['Montserrat', 'Outfit', sans-serif]"
                  >
                    Yes, Delete All
                  </button>
                  <button
                    onClick={() => setShowDeleteConfirmation(false)}
                    className={`px-4 py-2 rounded ${theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-200 text-gray-800'} font-['Montserrat', 'Outfit', sans-serif]`}
                  >
                    Cancel
                  </button>
                </div>
              ...
              <button
                ...
                className="flex items-center px-4 py-2 rounded bg-red-600 text-white font-['Montserrat', 'Outfit', sans-serif]"
              >
          ...
          <div className="flex justify-end space-x-2">
            ...
            <button
              type="submit"
              className={`px-4 py-2 rounded ${theme === 'dark' ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white'} font-['Montserrat', 'Outfit', sans-serif]`}
            >
              Save
            </button>
          </div>
